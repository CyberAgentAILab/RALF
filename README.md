...
```bash
poetry run python image2layout/preprocess_dataset/inpainting.py --dataset_root <DATASET_ROOT>
```

### Saliency detection

```bash
poetry run python image2layout/preprocess_dataset/saliency_detection.py --input_dir <INPUT_DIR> --output_dir <OUTPUT_DIR> (--algorithm (isnet|basnet))
```

### Aggregate data and dump to HFDS
```bash
poetry run python image2layout/preprocess_dataset/dump_dataset.py --dataset_root <DATASET_ROOT> --output_dir <OUTPUT_DIR>
```
